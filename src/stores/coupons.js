...
export default defineStore(
  'coupon',
  () => {
    const coupon=ref({}),coupons = ref([]), pagination = ref({})
    ...
    function addCoupon (coupon) {
      http.post(
        ...
        if (response.data.success) {
          ...
          response.data.message.forEach(function (msg) { pushMessage('danger', '新增優惠券失敗', msg) })
        }
      }).catch(function (error) {
        pushMessage('danger', '新增優惠券失敗', error?.message)
      }).finally(function () { getCoupons(1, 'admin') })
    }
    function editCoupon (coupon) {
      http.put(
        ...
        if (response.data.success) {
          ...
          response.data.message.forEach(function (msg) { pushMessage('danger', '修改優惠券失敗', msg) })
        }
      }).catch(function (error) {
    ...
    function getCoupons (page) {
      ...
      }).finally(function () { isLoading.value = false })
    }
      ...
      http.delete(`/api/${import.meta.env.VITE_PATH}/admin/coupon/${coupon.value.id}`)
        .then(function (response) {
          if (response.data.success) {
            ...
            response.data.message.forEach(function (msg) { pushMessage('danger', '移除優惠券失敗', msg) })
          }
        }).catch(function (error) {
